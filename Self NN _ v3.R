...
compute_cost <- function(Y, parameters, cache){
  num_layers <- length(cache) / 2
  prediction <- cache[[paste("A",num_layers,sep="")]]
  -1 / ncol(Y) * sum(Y * log(prediction) + (1-Y) * log(1-prediction))
}
...
for (i in 1:num_iter){
  ...
  J[i] <- compute_cost(data$Train$Y, parameters, cache)
